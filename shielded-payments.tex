...
The Credit Mode system links requests within a credit session---the commitment $\mathsf{C}$ is a pseudonym shared across all requests from the same account. For sensitive workloads (medical queries, legal analysis, investigative journalism), even pseudonymous linkability is unacceptable. The RLN Mode protocol eliminates intra-session linkability entirely: every request is cryptographically independent, with zero on-chain transactions per request.
